...
async function test() {
  try {
    const result = await sdk.financialAssistant('贵州茅台当前估值如何？');
    if (!result) {
      throw new Error('empty result');
    }
    console.log('✓ financialAssistant passed');
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes('暂不支持') || msg.includes('不支持') || msg.includes('服务暂时')) {
      console.log(`⚠ financialAssistant not supported: ${msg.slice(0, 80)}`);
    } else {
      console.log(`✗ financialAssistant failed: ${msg}`);
      process.exit(1);
    }
  }
}
