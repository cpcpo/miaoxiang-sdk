...
async function test() {
  try {
    const result = await sdk.stockDiagnosis('贵州茅台');
    if (!result) {
      throw new Error('empty result');
    }
    console.log('✓ stockDiagnosis passed');
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes('暂不支持') || msg.includes('不支持') || msg.includes('服务暂时')) {
      console.log(`⚠ stockDiagnosis not supported: ${msg.slice(0, 80)}`);
    } else {
      console.log(`✗ stockDiagnosis failed: ${msg}`);
      process.exit(1);
    }
  }
}
