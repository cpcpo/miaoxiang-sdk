...
async function test() {
  try {
    const result = await sdk.stockScreener('市盈率低于15的银行股');
    if (!result) {
      throw new Error('empty result');
    }
    console.log('✓ stockScreener passed');
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes('暂不支持') || msg.includes('不支持') || msg.includes('服务暂时')) {
      console.log(`⚠ stockScreener not supported: ${msg.slice(0, 80)}`);
    } else {
      console.log(`✗ stockScreener failed: ${msg}`);
      process.exit(1);
    }
  }
}
