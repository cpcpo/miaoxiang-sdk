...
async function test() {
  try {
    const result = await sdk.stockHotspot('今日热门板块');
    if (!result) {
      throw new Error('empty result');
    }
    console.log('✓ stockHotspot passed');
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes('暂不支持') || msg.includes('不支持') || msg.includes('服务暂时')) {
      console.log(`⚠ stockHotspot not supported: ${msg.slice(0, 80)}`);
    } else {
      console.log(`✗ stockHotspot failed: ${msg}`);
      process.exit(1);
    }
  }
}
