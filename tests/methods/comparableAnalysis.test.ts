...
async function test() {
  try {
    const result = await sdk.comparableAnalysis('宁德时代与比亚迪对比');
    if (!result) {
      throw new Error('empty result');
    }
    console.log('✓ comparableAnalysis passed');
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes('暂不支持') || msg.includes('不支持') || msg.includes('服务暂时')) {
      console.log(`⚠ comparableAnalysis not supported: ${msg.slice(0, 80)}`);
    } else {
      console.log(`✗ comparableAnalysis failed: ${msg}`);
      process.exit(1);
    }
  }
}
